...
export default function ComercioDetalle() {
  ...
  const rawLocation = comercio.location || comercio.address || "";
  const isMapsLink = rawLocation.startsWith("http");
  const mapsHref = comercio.lat && comercio.lng
    ...
    <div className="flex flex-col gap-4 pb-8">
      ...
      <div className="bg-slate-900/90 p-5 rounded-2xl border border-slate-800 shadow-lg flex flex-col gap-2">
        ...
        <p className="text-xs text-slate-400 mt-1">
          📍 {rawLocation || "Ubicación no especificada"} {comercio.sector ? `• ${comercio.sector}` : ""}
        </p>
      </div>
